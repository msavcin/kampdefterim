...
export function buildCampingMarkerHtml(
  ...
  const isKampfire = opts.variant === 'kampfireGold';
  ...
  const badgeAccentBorder = perfect
    ? isKampfire
      ...
      : isDark
        ...
        : 'rgba(44,42,38,0.35)'
    : badgeBorder;
  // For 5.0 ratings ensure sufficient contrast in both Kampfire (L4) light and dark modes.
  // - Kampfire + dark: use a light foreground
  // - Kampfire + light: use a darker foreground (readable on light badgeBg)
  const badgeAccentFg = perfect
    ? isKampfire
      ? (isDark ? '#F2EDE3' : '#2C2A26')
      : isDark
        ? '#F5F5F4'
        : '#2C2A26'
    : badgeFg;
